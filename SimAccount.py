class SimAccount:
    ...
    def entry_order(self, side, price, size, type, expire, pl, ls, i, dt, ut, tick_price):
        if self.order_side == '':
    ...
    def __check_expiration(self, i, dt, ut, tick_price):
        if self.ticks.ut[
            i] - self.order_ut >= self.order_expire and self.order_type != 'market' and self.order_type != 'losscut':
            self.__initialize_order()
    ...
    def __check_execution(self, i, dt, ut, tick_price):
        if ut - self.order_ut >= self.order_cancel_delay and self.order_side != '':
            if self.order_type == 'market' or self.order_type == 'losscut':
                ...
                self.__initialize_order()
            elif self.order_type == 'limit' and ((self.order_side == 'buy' and self.order_price >= tick_price) or (
                    self.order_side == 'sell' and self.order_price <= tick_price)):
    ...
    def __calc_executed_pl(self, exec_price, size, i):  # assume all order size was executed
        pl = (exec_price - self.holding_price - self.slip_page) * size if self.holding_side == 'buy' else (
                                                                                                                      self.holding_price - exec_price - self.slip_page) * size
